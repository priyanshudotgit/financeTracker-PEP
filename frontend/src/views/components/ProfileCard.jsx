...
const ProfileCard = ({
    ...
    return (
        ...
        <div className="relative z-10 h-full flex flex-col justify-between p-8 text-[var(--text-color,white)] bg-[var(--overlay-color,rgba(255,255,255,0.05))]">
            ...
            <div className="flex items-center gap-1.5 text-[15px] font-bold tracking-[0.1em] px-2 py-1 bg-green-600 rounded border border-green-950">
                <Lock size={14} className="opacity-100" />
                <span>User Profile</span>
            </div>
            <Activity className="opacity-100" size={20} />
            </div>

            <div className="flex-1 flex flex-col justify-end items-center text-center gap-6 mb-8">
            <div className="text-center">
                <h2 className="text-5xl font-extrabold tracking-[0.05em] m-0 mb-2 drop-shadow-md">{user?.displayName}</h2>
                <p className="text-xl tracking-[0.2em] opacity-100 m-0 uppercase">{user?.email}</p>
            </div>
            </div>

            <div className="flex justify-between items-end border-t border-white/20 pt-6">
            <div className="flex flex-col gap-1">
                <span className="text-[20px] tracking-[0.1em] opacity-100 text-black font-bold">ID NUMBER</span>
                <span className="font-normal text-xl tracking-[0.05em]">{user?.uid}</span>
            </div>
